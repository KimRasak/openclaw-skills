...
and transcribes the audio to text using a resident whisper model on GPU or CPU.
Optionally supports speaker diarization via WhisperX + pyannote.
...
is used instead of the resident faster-whisper model.
...
class TranscriberModel:
    """Keeps a whisper model loaded in memory (GPU or CPU).

    When diarize=True, uses WhisperX pipeline instead of resident faster-whisper.
    """
    ...
    def __init__(
        ...
        if diarize:
            import whisperx
            print(f"[TranscriberModel] Loading whisperx '{model_size}' on {self.device} ({self.compute_type}) ...")
            t0 = time.time()
            self.wx_model = whisperx.load_model(
                model_size, self.device, compute_type=self.compute_type,
            )
            self.model = None
            print(f"[TranscriberModel] WhisperX model loaded in {time.time() - t0:.1f}s")
        else:
            from faster_whisper import WhisperModel
            print(f"[TranscriberModel] Loading '{model_size}' on {self.device} ({self.compute_type}) ...")
            t0 = time.time()
    ...
    def transcribe_diarize(
        ...
        """WhisperX pipeline: transcribe → align → diarize → assign speakers."""
        import whisperx

        print(f"[Diarize] Transcribing with WhisperX ...")
        t0 = time.time()
        audio_array = whisperx.load_audio(audio_path)
        result = self.wx_model.transcribe(audio_array, batch_size=16, language=language)
        detected_lang = result.get("language", language or "zh")
        print(f"[Diarize] Transcription done in {time.time() - t0:.1f}s, "
              f"{len(result['segments'])} segments, lang={detected_lang}")

        print(f"[Diarize] Aligning to word-level timestamps ...")
        t0 = time.time()
        model_a, metadata = whisperx.load_align_model(
            language_code=detected_lang, device=self.device,
        )
        result = whisperx.align(
            result["segments"], model_a, metadata, audio_array, self.device,
            return_char_alignments=False,
        )
        del model_a
        print(f"[Diarize] Alignment done in {time.time() - t0:.1f}s")
        ...
        print(f"[Diarize] Diarization done in {time.time() - t0:.1f}s")
        ...
        return _merge_consecutive_speakers(segments)
...
def segments_to_text(segments: list[dict], timestamps: bool = True, diarize: bool = False) -> str:
    lines = []
    for seg in segments:
        parts: list[str] = []
...
_server_diarize: bool = False
_server_num_speakers: int | None = None
    ...
    try:
        ...
        async with _transcribe_lock:
            ...
            try:
                ...
                # --- Step 3: Transcribe (+ diarize if enabled) ---
                step_label = "转录 + 说话人分离" if _server_diarize else "转录"
                await ws_send_safe(websocket, {"type": "step", "id": "step-transcribe", "state": "active"})
                await ws_send_safe(websocket, {"type": "status", "data": f"正在{step_label}..."})
                await ws_send_safe(websocket, {"type": "log", "data": f"开始{step_label}..."})

                t0 = time.time()
                try:
                    if _server_diarize:
                        _diarize_fn = functools.partial(
                            transcriber.transcribe_diarize,
                            str(media_path),
                            num_speakers=_server_num_speakers,
                        )
                        segments = await loop.run_in_executor(None, _diarize_fn)
                    else:
                ...
                await ws_send_safe(websocket, {
                    "type": "log",
                    "data": f"{step_label}完成: {len(segments)} 段, 耗时 {elapsed_tr:.1f}s",
                })

                result_text = segments_to_text(segments, timestamps=timestamps, diarize=_server_diarize)
                await ws_send_safe(websocket, {
...
def main():
    ...
    parser.add_argument("--cpu", action="store_true",
                        help="Run model on CPU (int8) instead of CUDA GPU")
    parser.add_argument("--diarize", action="store_true",
                        help="Enable speaker diarization via WhisperX + pyannote (requires HF token)")
    ...
    if args.diarize:
        if not hf_token:
            ...
            if os.path.isfile(cached):
                hf_token = open(cached).read().strip()
        if not hf_token:
            print(
                "Error: --diarize requires a HuggingFace token.\n"
                "  Set HF_TOKEN env var, pass --hf-token <token>, or login via huggingface-cli.\n"
    ...
    global transcriber, _server_diarize, _server_num_speakers
    _server_diarize = args.diarize
    _server_num_speakers = args.num_speakers
